

#################################################################################

# rm(list=ls())

suppressPackageStartupMessages({
  library(ipumsr)
  library(dplyr, warn.conflicts = FALSE)  
  library(sf)
  library(tidyr)
  library(devtools)
  library(ggplot2)
  library(mapview)
  library(stringi)
  library(tidyverse)
  library(leaflet)
  library(purrr)
  library(geosphere)
  library(rmapshaper)
  library(htmltools)
  library(scales)
  library(tmap)
  library(RColorBrewer)
  library(psych)
  library(reshape)
  library(rlang)
  library(purrr)
  library(timetk)
  library(kableExtra)
  library(highcharter)
  library(PerformanceAnalytics)
  library(httr)
  library(jsonlite)
  library(geojsonio)
  library(sp)
  library(mapview)
  library(tidycensus)
  library(tidyverse)
  library(tigris)
  library(rgdal)
  library(downloader)
  library(geojsonio)
  require(quantmod)
  options(tigris_class = "sf")
  options(tigris_use_cache = TRUE)
})
theme_set(theme_bw())

# Set file path
setwd("G:/Shared drives/Projects/5035_Tech Equity/Data/A_IPUMS_Data")

#################################################################################

# Creating charts of IPUMS data

cps_ddi7 <- read_ipums_ddi("usa_00007.xml")
cps_data7 <- read_ipums_micro(cps_ddi7, verbose = FALSE)

cps_ddi6 <- read_ipums_ddi("usa_00006.xml")
cps_data6 <- read_ipums_micro(cps_ddi6, verbose = FALSE)

cps_ddi5 <- read_ipums_ddi("usa_00005.xml")
cps_data5 <- read_ipums_micro(cps_ddi5, verbose = FALSE)

cps_ddi4 <- read_ipums_ddi("usa_00004.xml")
cps_data4 <- read_ipums_micro(cps_ddi4, verbose = FALSE)

cps_data5$STATEFIP = stri_pad_left(cps_data5$STATEFIP, 2, "0")
cps_data5$COUNTYFIP = stri_pad_left(cps_data5$COUNTYFIP, 3, "0")
cps_data5$PUMA = stri_pad_left(cps_data5$PUMA, 5, "0")

cps_data4$STATEFIP = stri_pad_left(cps_data4$STATEFIP, 2, "0")
cps_data4$COUNTYFIP = stri_pad_left(cps_data4$COUNTYFIP, 3, "0")
cps_data4$PUMA = stri_pad_left(cps_data4$PUMA, 5, "0")

csvIPUMS_PUMAs <- read.csv(file = 'G:/Shared drives/Projects/5035_Tech Equity/Data/A_CSV/IPUMS_Untrimmed/IPUMS_PUMAs_Untrimmed_csv.csv')
View(csvIPUMS_PUMAs)






#################################################################################


# June 15th 

# 1. Tech Equity Initiative
#     1) Create Change Census Broadband Map
#     1-1. % without broadband in the home by census tract in 2017 (ACS-5)
#     1-2. % without broadband in the home by census tract in 2019 (ACS-5)

geoACS_BRDBD1917_Tract = st_read("G:/Shared drives/Projects/5035_Tech Equity/Data/A_Shapefile/Broadband_Data/FCC/ACS1917_BRDPCT.shp")

View(geoACS_BRDBD1917_Tract2)
colnames(geoACS_BRDBD1917_Tract)

geoACS_BRDBD1917_Tract2 <-
geoACS_BRDBD1917_Tract %>%
  mutate(
    POP19_N = POP19_T - POP19_B,
    PCT19_N = 100 - PCT19_B,
    POP17_N = POP17_T - POP17_B,
    PCT17_N = 100 - PCT17_B
  ) 

st_write(geoACS_BRDBD1917_Tract2,
         "G:/Shared drives/Projects/5035_Tech Equity/Data/A_Shapefile/Broadband_Data/FCC/ACS1917_BRDPCT2.shp", 
         driver = "ESRI Shapefile")
geoACS_BRDBD1917_Tract2 = st_read("G:/Shared drives/Projects/5035_Tech Equity/Data/A_Shapefile/Broadband_Data/FCC/ACS1917_BRDPCT2.shp")


#     2) Create Change Census Broadband Map
#     2-1. % of percentage of households in census blocks with at least 100Mbps in 2016 by census tract
#     2-2. % of percentage of households in census blocks with at least 100Mbps in 2020 by census tract

FCC_DPL20_NY <- read.csv(file = 'G:/Shared drives/Projects/5035_Tech Equity/Data/A_CSV/FCC/NY-Fixed-Jun2020-v1.csv')
FCC_DPL20_NJ <- read.csv(file = 'G:/Shared drives/Projects/5035_Tech Equity/Data/A_CSV/FCC/NJ-Fixed-Jun2020-v1.csv')
FCC_DPL20_CT <- read.csv(file = 'G:/Shared drives/Projects/5035_Tech Equity/Data/A_CSV/FCC/CT-Fixed-Jun2020-v1.csv')

FCC_DPL16_NY <- read.csv(file = 'G:/Shared drives/Projects/5035_Tech Equity/Data/A_CSV/FCC/NY-Fixed-Jun2016-v4.csv')
FCC_DPL16_NJ <- read.csv(file = 'G:/Shared drives/Projects/5035_Tech Equity/Data/A_CSV/FCC/NJ-Fixed-Jun2016-v4.csv')
FCC_DPL16_CT <- read.csv(file = 'G:/Shared drives/Projects/5035_Tech Equity/Data/A_CSV/FCC/CT-Fixed-Jun2016-v4.csv')

FCC_DPL20_NY$BlockCode = stri_pad_left(FCC_DPL20_NY$BlockCode, 15)
FCC_DPL16_NY$BlockCode = stri_pad_left(FCC_DPL16_NY$BlockCode, 15)
FCC_DPL20_NJ$BlockCode = stri_pad_left(FCC_DPL20_NJ$BlockCode, 15)
FCC_DPL16_NJ$BlockCode = stri_pad_left(FCC_DPL16_NJ$BlockCode, 15)
FCC_DPL20_CT$BlockCode = stri_pad_left(FCC_DPL20_CT$BlockCode, 15, "0")
FCC_DPL16_CT$BlockCode = stri_pad_left(FCC_DPL16_CT$BlockCode, 15, "0")

FCC_DPL20_TRI <- rbind(FCC_DPL20_NY, FCC_DPL20_NJ, FCC_DPL20_CT)
FCC_DPL16_TRI <- rbind(FCC_DPL16_NY, FCC_DPL16_NJ, FCC_DPL16_CT)

# Select Only Max Speed in Census Blocks
FCC_MAX20_LES100 <-
  FCC_DPL20_TRI %>%
  dplyr::rename(
    TecCode20 = "TechCode",
    Consumr20 = "Consumer",
    MaxAdDn20 = "MaxAdDown",
    MaxAdUp20 = "MaxAdUp",
    Busines20 = "Business"
  ) %>%
  select(BlockCode:Busines20) %>%
  group_by(BlockCode) %>%
  slice(which.max(MaxAdDn20)) %>%
  dplyr::rename(
    GEOID10 = "BlockCode"
  ) %>%
  select(GEOID10, TecCode20, MaxAdDn20) %>%
  mutate(
    MaxAdDn20_LES100 = case_when(
      MaxAdDn20 >= 100 ~ 0,
      MaxAdDn20 < 100 ~ 1
  )) 

FCC_MAX16_LES100 <-
  FCC_DPL16_TRI %>%
  dplyr::rename(
    TecCode16 = "TechCode",
    Consumr16 = "Consumer",
    MaxAdDn16 = "MaxAdDown",
    MaxAdUp16 = "MaxAdUp",
    Busines16 = "Business",
    MxCIRDn16 = "MaxCIRDown",
    MxCIRUp16 = "MaxCIRUp"
  ) %>%
  select(BlockCode:MxCIRUp16) %>%
  group_by(BlockCode) %>%
  slice(which.max(MaxAdDn16)) %>%
  dplyr::rename(
    GEOID10 = "BlockCode"
  ) %>%
  select(GEOID10, TecCode16, MaxAdDn16) %>%
  mutate(
    MaxAdDn16_LES100 = case_when(
      MaxAdDn16 >= 100 ~ 0,
      MaxAdDn16 < 100 ~ 1
    )) 

# Census Block Population Data
census_api_key("5e8ebd0bdf69ccee12f6a6be2cdc51acae7b2339", overwrite = TRUE, install = TRUE)

sf1_10_ny <- get_decennial(geography = "block", 
                           variables = c("P001001" # Total Population
                           ),
                           state = "NY", county = c("005", "027", "047", "059", "061",
                                                    "071", "079", "081", "085", "087",
                                                    "103", "105", "111", "119"),
                           survey = "sf1", year = 2010)
sf1_10_nj <- get_decennial(geography = "block",
                           variables = c("P001001" # Total Population
                           ),
                           state = "NJ", county = c("003", "013", "017", "019", "021",
                                                    "023", "025", "027", "029", "031", 
                                                    "035", "037", "039", "041"),
                           survey = "sf1", year = 2010)
sf1_10_ct <- get_decennial(geography = "block",
                           variables = c("P001001" # Total Population
                           ),
                           state = "CT", county = c("001", "005", "009"),
                           survey = "sf1", year = 2010)
SF1_10_TRI <- rbind(sf1_10_ny, sf1_10_nj, sf1_10_ct)
SF1_10_TRI_POP <-
  SF1_10_TRI %>% 
  cast(GEOID~variable) %>%
  dplyr::rename(
    GEOID10 = "GEOID",
    SF1_POP10 = "P001001"
  ) 

# Merge Census Blocks in RPA Region with Population File
geo_Blocks = st_read("G:/Shared drives/Projects/5035_Tech Equity/Geographies/RPA_Blocks_shp.shp")
geo_Tracts = st_read("G:/Shared drives/Projects/5035_Tech Equity/Geographies/RPA_tracts_shp.shp")
csv_Blocks <- read.csv(file = 'G:/Shared drives/Projects/5035_Tech Equity/Geographies/RPA_Blocks_ref.csv')
csv_Blocks$GEOID10 = stri_pad_left(csv_Blocks$GEOID10, 15, "0")
csvSF1_POP10 <- merge(csv_Blocks, SF1_10_TRI_POP, by = "GEOID10", all.x = TRUE)

# Create Census Tracts Reference Column 
csvSF1_POP10_2 <- 
  csvSF1_POP10 %>%
  select(GEOID10, SF1_POP10) %>%
  mutate(
    TRACTID = GEOID10
  ) %>%
  relocate(TRACTID)
csvSF1_POP10_2$TRACTID <- gsub(".{4}$", "", csvSF1_POP10_2$TRACTID)

# Merge with Population csv file, FCC areas with less than 100 Mbps
csvFCC_POP10_LES100 = list (csvSF1_POP10_2, FCC_MAX20_LES100, FCC_MAX16_LES100) %>%
  reduce(left_join, by = "GEOID10", all.x = TRUE)
View(csvFCC_POP10_LES100)

# Summarize and Combine
csvFCC_POPLES100_SUM1 <-
  csvFCC_POP10_LES100 %>%
  group_by(TRACTID) %>%
  summarise(POP_TTL = sum(SF1_POP10))

csvFCC_POPLES100_SUM2 <-
  csvFCC_POP10_LES100 %>%
  filter(MaxAdDn20_LES100 == 1) %>%
  group_by(TRACTID) %>%
  summarise(POP20_LES100 = sum(SF1_POP10))

csvFCC_POPLES100_SUM3 <-
  csvFCC_POP10_LES100 %>%
  filter(MaxAdDn16_LES100 == 1) %>%
  group_by(TRACTID) %>%
  summarise(POP16_LES100 = sum(SF1_POP10))  
  
csvFCC_POPLES100_SUM_TTL = list (csvFCC_POPLES100_SUM1, csvFCC_POPLES100_SUM2, csvFCC_POPLES100_SUM3) %>%
  reduce(left_join, by = "TRACTID", all.x = TRUE)
csvFCC_POPLES100_SUM_TTL <- mutate_all(csvFCC_POPLES100_SUM_TTL, ~replace(., is.na(.), 0))

# Make new columns of percentage 
csvFCC_POPLES100_SUM_TTL2 <-
  csvFCC_POPLES100_SUM_TTL %>%
  mutate( 
    PCT20_LES100 = (POP20_LES100/POP_TTL)*100, 
    PCT16_LES100 = (POP16_LES100/POP_TTL)*100
  ) %>%
  mutate_at(vars(PCT20_LES100, PCT16_LES100), 
            funs(round(., 1))) %>%
  dplyr::rename(
    GEOID = "TRACTID"
  )

View(csvFCC_POPLES100_SUM_TTL2)
colnames(csvFCC_POPLES100_SUM_TTL2)


# Merge with Shapefile and Create layers
geoFCC_POPLES100_SUM_TTL <- merge(geo_Tracts, csvFCC_POPLES100_SUM_TTL2, by = "GEOID", all.x = TRUE)
st_write(geoFCC_POPLES100_SUM_TTL,
         "G:/Shared drives/Projects/5035_Tech Equity/Data/A_Shapefile/Broadband_Data/FCC/FCC_POPLES100_SUM_TTL.shp", 
         driver = "ESRI Shapefile")



#################################################################################


# 1. Aggregate LINKED_Public.csv data by:
#     1) Origin Tract (OTRACT)
#     2) Destination Tract (DTRACT)
#     3) Time of Departure (TOD_R)
#     4) Primary Mode, short version (PMODE1)
#           - Use rts_variable_lookup to recode
#     6) Origin to Destination Primary Trip Purpose (ODTPURP)
#     7) Household Income Range (INCOM_R)

# Use the aggregated data to generate some summary tables:
#     1) Work and Non-work Trips to Manhattan (36061) by Origin County
#     2) Number of Auto, Transit, Other trips by Origin County
#     3) Number of Auto, Transit, Other trips by Destination County
#     4) Number of Auto, Transit, Other trips by Household Income by RPA Subregion


# Read LINKED_Public.csv
Linked_Public <- read.csv(file = 'G:/Shared drives/Projects/525_Workforce/Post-Covid Recovery/Data/NYMTC RTS/LINKED_Public.csv')
Relabel <- read.csv(file = 'G:/Shared drives/Projects/525_Workforce/Post-Covid Recovery/Data/NYMTC RTS/rts_variable_lookup.csv')

View(Linked_Public)
colnames(Linked_Public)
summary(Linked_Public)


# Aggregate Example
Linked_Public_Sub1 <-
Linked_Public %>%
  select(OCOUNTY, DCOUNTY, HCOUNTY,
         PMODE1, ODTPURP, TOD_R, INCOM_R) %>%
  mutate(PMODE1_RE1 = case_when(
    PMODE1 == 1 ~ 2, # 2. Bus
    PMODE1 == 2 ~ 1, # 1. Auto
    PMODE1 == 3 ~ 4, # 4. Commuter Rail
    PMODE1 == 4 ~ 2,
    PMODE1 == 5 ~ 5, # 5. Ferry
    PMODE1 == 6 ~ 3, # 3. Rail
    PMODE1 == 7 ~ 3,
    PMODE1 == 8 ~ 2,
    PMODE1 == 9 ~ 2,
    PMODE1 == 10 ~ 1,
    PMODE1 == 11 ~ 1,
    PMODE1 == 12 ~ 6, # 6. Bike
    PMODE1 == 13 ~ 7, # 7. Walk
    PMODE1 == 97 ~ 8  # 8. Other
    ),
    PMODE1_RE2 = case_when(
      PMODE1 == 1 ~ 2, # 2. Transit
      PMODE1 == 2 ~ 1, # 1. Auto
      PMODE1 == 3 ~ 2, 
      PMODE1 == 4 ~ 2,
      PMODE1 == 5 ~ 2, 
      PMODE1 == 6 ~ 2, 
      PMODE1 == 7 ~ 2,
      PMODE1 == 8 ~ 2,
      PMODE1 == 9 ~ 2,
      PMODE1 == 10 ~ 1,
      PMODE1 == 11 ~ 1,
      PMODE1 == 12 ~ 3, # 3. Other
      PMODE1 == 13 ~ 3, 
      PMODE1 == 97 ~ 3  
    ),
    ODTPURP_RE1 = case_when(
      ODTPURP == 1 ~ 1, # 1. Work Trip
      ODTPURP == 2 ~ 0, # 0. Non-work Trip
      ODTPURP == 3 ~ 0, 
      ODTPURP == 4 ~ 1,
      ODTPURP == 5 ~ 0, 
      ODTPURP == 6 ~ 0, 
      ODTPURP == 7 ~ 1,
      ODTPURP == 8 ~ 0,
      ODTPURP == 9 ~ 0  
    ),
    SUBREGN = case_when(
      HCOUNTY == 09001 ~ 5, # 5. Connecticut
      HCOUNTY == 09009 ~ 5, 
      HCOUNTY == 34003 ~ 4, # 4. New Jersey 
      HCOUNTY == 34013 ~ 4,
      HCOUNTY == 34017 ~ 4, 
      HCOUNTY == 34019 ~ 4, 
      HCOUNTY == 34021 ~ 4,
      HCOUNTY == 34023 ~ 4,
      HCOUNTY == 34025 ~ 4,
      HCOUNTY == 34027 ~ 4, 
      HCOUNTY == 34029 ~ 4,
      HCOUNTY == 34031 ~ 4,
      HCOUNTY == 34035 ~ 4,
      HCOUNTY == 34037 ~ 4,
      HCOUNTY == 34039 ~ 4,
      HCOUNTY == 34041 ~ 4,
      HCOUNTY == 36005 ~ 1, # 1. New York City
      HCOUNTY == 36027 ~ 3, # 3. Other New York
      HCOUNTY == 36047 ~ 1, 
      HCOUNTY == 36059 ~ 2, # 2. Long Island
      HCOUNTY == 36061 ~ 1, 
      HCOUNTY == 36071 ~ 3,
      HCOUNTY == 36079 ~ 3,
      HCOUNTY == 36081 ~ 1,
      HCOUNTY == 36085 ~ 1, 
      HCOUNTY == 36087 ~ 3, 
      HCOUNTY == 36103 ~ 2, 
      HCOUNTY == 36119 ~ 3
    ))

write.csv(Linked_Public_Sub1, file = 'G:/Shared drives/Projects/5035_Tech Equity/Data/A_CSV/Public_Linked/Linked_Public_Sub1.csv')
Linked_Public_Sub1 <- read.csv(file = 'G:/Shared drives/Projects/5035_Tech Equity/Data/A_CSV/Public_Linked/Linked_Public_Sub1.csv')
Linked_Public_Sub1$OCOUNTY = stri_pad_left(Linked_Public_Sub1$OCOUNTY, 5, "0")
Linked_Public_Sub1$DCOUNTY = stri_pad_left(Linked_Public_Sub1$DCOUNTY, 5, "0")
Linked_Public_Sub1 <-
  Linked_Public_Sub1 %>%
  select(OCOUNTY:SUBREGN)

# 1. Work and Non-work Trips to Manhattan (36061) by Origin County
Sub1_Answer1 <-
Linked_Public_Sub1 %>%
  select(DCOUNTY, OCOUNTY, ODTPURP_RE1) %>%
  filter(DCOUNTY == 36061) %>%
  group_by(DCOUNTY, OCOUNTY) %>%
  count(ODTPURP_RE1) %>%
  dplyr::rename(
    Count = "n"
  ) %>%
  spread(ODTPURP_RE1, Count) %>%
  dplyr::rename(
    NWRK_TP = "0",
    YWRK_TP = "1"
  ) %>%
  mutate(
    TOTAL_TP = NWRK_TP + YWRK_TP,
    NWRK_PCT = (NWRK_TP/TOTAL_TP) * 100,
    YWRK_PCT = (YWRK_TP/TOTAL_TP) * 100
  ) 
write.csv(Sub1_Answer1, file = 'G:/Shared drives/Projects/5035_Tech Equity/Data/A_CSV/Public_Linked/Sub1_Answer1.csv')
Sub1_Answer1 <- read.csv(file = 'G:/Shared drives/Projects/5035_Tech Equity/Data/A_CSV/Public_Linked/Sub1_Answer1.csv')
Sub1_Answer1$DCOUNTY = stri_pad_left(Sub1_Answer1$DCOUNTY, 5, "0")
Sub1_Answer1$OCOUNTY = stri_pad_left(Sub1_Answer1$OCOUNTY, 5, "0")
Sub1_Answer1 <-
  Sub1_Answer1 %>%
  select(DCOUNTY:TOTAL_TP)

# 2. Number of Auto, Transit, Other trips by Origin County
Sub1_Answer2 <-
Linked_Public_Sub1 %>%
  select(OCOUNTY, PMODE1_RE2) %>%
  group_by(OCOUNTY) %>%
  count(PMODE1_RE2) %>%
  dplyr::rename(
    Count = "n"
  ) %>%
  cast(OCOUNTY ~ PMODE1_RE2) %>%
  dplyr::rename(
    AUTO_TP = "1",
    TRST_TP = "2",
    OTHR_TP = "3"
  ) %>%
  mutate(
    TOTAL_TP = AUTO_TP + TRST_TP + OTHR_TP,
    AUTO_PCT = (AUTO_TP/TOTAL_TP) * 100,
    TRST_PCT = (TRST_TP/TOTAL_TP) * 100,
    OTHR_PCT = (OTHR_TP/TOTAL_TP) * 100
  ) 
write.csv(Sub1_Answer2, file = 'G:/Shared drives/Projects/5035_Tech Equity/Data/A_CSV/Public_Linked/Sub1_Answer2.csv')
Sub1_Answer2 <- read.csv(file = 'G:/Shared drives/Projects/5035_Tech Equity/Data/A_CSV/Public_Linked/Sub1_Answer2.csv')
Sub1_Answer2$OCOUNTY = stri_pad_left(Sub1_Answer2$OCOUNTY, 5, "0")
Sub1_Answer2 <-
  Sub1_Answer2 %>%
  select(OCOUNTY:TOTAL_TP)

# 3. Number of Auto, Transit, Other trips by Destination County
Sub1_Answer3 <-
Linked_Public_Sub1 %>%
  select(DCOUNTY, PMODE1_RE2) %>%
  group_by(DCOUNTY) %>%
  count(PMODE1_RE2) %>%
  dplyr::rename(
    Count = "n"
  ) %>%
  cast(DCOUNTY ~ PMODE1_RE2) %>%
  dplyr::rename(
    AUTO_TP = "1",
    TRST_TP = "2",
    OTHR_TP = "3"
  ) %>%
  mutate(
    TOTAL_TP = AUTO_TP + TRST_TP + OTHR_TP,
    AUTO_PCT = (AUTO_TP/TOTAL_TP) * 100,
    TRST_PCT = (TRST_TP/TOTAL_TP) * 100,
    OTHR_PCT = (OTHR_TP/TOTAL_TP) * 100
  )
write.csv(Sub1_Answer3, file = 'G:/Shared drives/Projects/5035_Tech Equity/Data/A_CSV/Public_Linked/Sub1_Answer3.csv')
Sub1_Answer3 <- read.csv(file = 'G:/Shared drives/Projects/5035_Tech Equity/Data/A_CSV/Public_Linked/Sub1_Answer3.csv')
Sub1_Answer3$DCOUNTY = stri_pad_left(Sub1_Answer3$DCOUNTY, 5, "0")
Sub1_Answer3 <-
  Sub1_Answer3 %>%
  select(DCOUNTY:TOTAL_TP)

# 4. Number of Auto, Transit, Other trips by Household Income by RPA Subregion
Sub1_Answer4 <-
Linked_Public_Sub1 %>%
  select(SUBREGN, PMODE1_RE2, INCOM_R) %>%
  group_by(SUBREGN, INCOM_R) %>%  
  count(PMODE1_RE2) %>%
  dplyr::rename(
    Count = "n"
  ) %>%
  spread(PMODE1_RE2, Count) %>%
  dplyr::rename(
    AUTO_TP = "1",
    TRST_TP = "2",
    OTHR_TP = "3"
  ) %>%
  mutate(
    TOTAL_TP = AUTO_TP + TRST_TP + OTHR_TP,
    AUTO_PCT = (AUTO_TP/TOTAL_TP) * 100,
    TRST_PCT = (TRST_TP/TOTAL_TP) * 100,
    OTHR_PCT = (OTHR_TP/TOTAL_TP) * 100
  ) 
write.csv(Sub1_Answer4, file = 'G:/Shared drives/Projects/5035_Tech Equity/Data/A_CSV/Public_Linked/Sub1_Answer4.csv')
Sub1_Answer4 <- read.csv(file = 'G:/Shared drives/Projects/5035_Tech Equity/Data/A_CSV/Public_Linked/Sub1_Answer4.csv')
Sub1_Answer4 <-
  Sub1_Answer4 %>%
  select(SUBREGN:TOTAL_TP)



# Additional Findings

# 18. OTAZ       Origin TAZ (NYMTC; WGS84)
# 19. DTAZ       Destination TAZ (NYMTC; WGS84)
# 79. TRPDIST_HN    Hwy Network Trip Distance (Miles)
# 80. TRPDUR_HN     Trip Duration (Minutes) - Highway Network 
# 81. OTRACT        Origin Tract 2010 (WGS84)
# 82. DTRACT        Destination Tract 2010 (WGS84)
# 123. WORK_PURP    Tour Purpose; Work vs Non-work. Based on TOUR_PURP
# 129. WORKTRIP     Trip Purpose; Dummy for work trip; Re-categorized varialbe based on ODTPURP2_R

Linked_Public_Sub2 <-
  Linked_Public %>%
  select(OCOUNTY, DCOUNTY, HCOUNTY,
         PMODE1, ODTPURP, TOD_R, INCOM_R,
         TRPDIST_HN, TRPDUR_HN, WORK_PURP) %>%
  mutate(PMODE1_RE1 = case_when(
    PMODE1 == 1 ~ 2, # 2. Bus
    PMODE1 == 2 ~ 1, # 1. Auto
    PMODE1 == 3 ~ 4, # 4. Commuter Rail
    PMODE1 == 4 ~ 2,
    PMODE1 == 5 ~ 5, # 5. Ferry
    PMODE1 == 6 ~ 3, # 3. Rail
    PMODE1 == 7 ~ 3,
    PMODE1 == 8 ~ 2,
    PMODE1 == 9 ~ 2,
    PMODE1 == 10 ~ 1,
    PMODE1 == 11 ~ 1,
    PMODE1 == 12 ~ 6, # 6. Bike
    PMODE1 == 13 ~ 7, # 7. Walk
    PMODE1 == 97 ~ 8  # 8. Other
  ),
  PMODE1_RE2 = case_when(
    PMODE1 == 1 ~ 2, # 2. Transit
    PMODE1 == 2 ~ 1, # 1. Auto
    PMODE1 == 3 ~ 2, 
    PMODE1 == 4 ~ 2,
    PMODE1 == 5 ~ 2, 
    PMODE1 == 6 ~ 2, 
    PMODE1 == 7 ~ 2,
    PMODE1 == 8 ~ 2,
    PMODE1 == 9 ~ 2,
    PMODE1 == 10 ~ 1,
    PMODE1 == 11 ~ 1,
    PMODE1 == 12 ~ 3, # 3. Other
    PMODE1 == 13 ~ 3, 
    PMODE1 == 97 ~ 3  
  ),
  ODTPURP_RE1 = case_when(
    ODTPURP == 1 ~ 1, # 1. Work Trip
    ODTPURP == 2 ~ 0, # 0. Non-work Trip
    ODTPURP == 3 ~ 0, 
    ODTPURP == 4 ~ 1,
    ODTPURP == 5 ~ 0, 
    ODTPURP == 6 ~ 0, 
    ODTPURP == 7 ~ 1,
    ODTPURP == 8 ~ 0,
    ODTPURP == 9 ~ 0  
  ),
  SUBREGN = case_when(
    HCOUNTY == 09001 ~ 5, # 5. Connecticut
    HCOUNTY == 09009 ~ 5, 
    HCOUNTY == 34003 ~ 4, # 4. New Jersey 
    HCOUNTY == 34013 ~ 4,
    HCOUNTY == 34017 ~ 4, 
    HCOUNTY == 34019 ~ 4, 
    HCOUNTY == 34021 ~ 4,
    HCOUNTY == 34023 ~ 4,
    HCOUNTY == 34025 ~ 4,
    HCOUNTY == 34027 ~ 4, 
    HCOUNTY == 34029 ~ 4,
    HCOUNTY == 34031 ~ 4,
    HCOUNTY == 34035 ~ 4,
    HCOUNTY == 34037 ~ 4,
    HCOUNTY == 34039 ~ 4,
    HCOUNTY == 34041 ~ 4,
    HCOUNTY == 36005 ~ 1, # 1. New York City
    HCOUNTY == 36027 ~ 3, # 3. Other New York
    HCOUNTY == 36047 ~ 1, 
    HCOUNTY == 36059 ~ 2, # 2. Long Island
    HCOUNTY == 36061 ~ 1, 
    HCOUNTY == 36071 ~ 3,
    HCOUNTY == 36079 ~ 3,
    HCOUNTY == 36081 ~ 1,
    HCOUNTY == 36085 ~ 1, 
    HCOUNTY == 36087 ~ 3, 
    HCOUNTY == 36103 ~ 2, 
    HCOUNTY == 36119 ~ 3
  ))

Linked_Public_Sub2$DCOUNTY = stri_pad_left(Linked_Public_Sub2$DCOUNTY, 5, "0")
Linked_Public_Sub2$OCOUNTY = stri_pad_left(Linked_Public_Sub2$OCOUNTY, 5, "0")
Linked_Public_Sub2$HCOUNTY = stri_pad_left(Linked_Public_Sub2$HCOUNTY, 5, "0")
  
# 5-1. Percentage of Auto, Transit, Other trips by Work and Non-work Trips by Origin County
# 5-2. Percentage of Auto, Transit, Other trips to Manhattan by Work and Non-work Trips by Origin County

Sub2_Answer5.1 <- 
Linked_Public_Sub2 %>%
  select(OCOUNTY, WORK_PURP, PMODE1_RE2) %>%
  group_by(OCOUNTY, WORK_PURP) %>%
  count(PMODE1_RE2) %>%
  dplyr::rename(
    Count = "n"
  ) %>%
  spread(PMODE1_RE2, Count) %>%
  dplyr::rename(
    AUTO_TP = "1",
    TRST_TP = "2",
    OTHR_TP = "3"
  ) %>%
  mutate(
    TOTAL_TP = AUTO_TP + TRST_TP + OTHR_TP,
    AUTO_PCT = (AUTO_TP/TOTAL_TP) * 100,
    TRST_PCT = (TRST_TP/TOTAL_TP) * 100,
    OTHR_PCT = (OTHR_TP/TOTAL_TP) * 100
  )

Sub2_Answer5.2 <- 
Linked_Public_Sub2 %>%
  filter(DCOUNTY == 36061) %>%
  select(OCOUNTY, WORK_PURP, PMODE1_RE2) %>%
  group_by(OCOUNTY, WORK_PURP) %>%
  count(PMODE1_RE2) %>%
  dplyr::rename(
    Count = "n"
  ) %>%
  spread(PMODE1_RE2, Count) %>%
  dplyr::rename(
    AUTO_TP = "1",
    TRST_TP = "2",
    OTHR_TP = "3"
  ) %>%
  mutate_at(c("AUTO_TP", "TRST_TP", "OTHR_TP"), ~replace(., is.na(.), 0)) %>%
  mutate(
    TOTAL_TP = AUTO_TP + TRST_TP + OTHR_TP,
    AUTO_PCT = (AUTO_TP/TOTAL_TP) * 100,
    TRST_PCT = (TRST_TP/TOTAL_TP) * 100,
    OTHR_PCT = (OTHR_TP/TOTAL_TP) * 100
  )



#################################################################################


# Selected Data Description

# 95. HCOUNTY       Home County FIPS
# 09001  Fairfield
# 09009  New Haven
# 34003  Bergen
# 34013  Essex
# 34017  Hudson
# 34019  Hunterdon
# 34021  Mercer
# 34023  Middlesex
# 34025  Monmouth
# 34027  Morris
# 34029  Ocean
# 34031  Passaic
# 34035  Somerset
# 34037  Sussex
# 34039  Union
# 34041  Warren
# 36005  Bronx
# 36027  Dutchess
# 36047  Brooklyn (Kings)
# 36059  Nassau
# 36061  Manhattan (New York)
# 36071  Orange
# 36079  Putnam
# 36081  Queens
# 36085  Staten Island (Richmond)
# 36087  Rockland
# 36103  Suffolk
# 36119  Westchester

# 115. OCOUNTY      Origin County FIPS
# 09001  Fairfield
# 09009  New Haven
# 34003  Bergen
# 34013  Essex
# 34017  Hudson
# 34019  Hunterdon
# 34021  Mercer
# 34023  Middlesex
# 34025  Monmouth
# 34027  Morris
# 34029  Ocean
# 34031  Passaic
# 34035  Somerset
# 34037  Sussex
# 34039  Union
# 34041  Warren
# 36005  Bronx
# 36027  Dutchess
# 36047  Brooklyn (Kings)
# 36059  Nassau
# 36061  Manhattan (New York)
# 36071  Orange
# 36079  Putnam
# 36081  Queens
# 36085  Staten Island (Richmond)
# 36087  Rockland
# 36103  Suffolk
# 36119  Westchester
# 0      Location Outside of NYBPM Area

# 116. DCOUNTY      Destination County FIPS
# 09001  Fairfield
# 09009  New Haven
# 34003  Bergen
# 34013  Essex
# 34017  Hudson
# 34019  Hunterdon
# 34021  Mercer
# 34023  Middlesex
# 34025  Monmouth
# 34027  Morris
# 34029  Ocean
# 34031  Passaic
# 34035  Somerset
# 34037  Sussex
# 34039  Union
# 34041  Warren
# 36005  Bronx
# 36027  Dutchess
# 36047  Brooklyn (Kings)
# 36059  Nassau
# 36061  Manhattan (New York)
# 36071  Orange
# 36079  Putnam
# 36081  Queens
# 36085  Staten Island (Richmond)
# 36087  Rockland
# 36103  Suffolk
# 36119  Westchester
# 0      Location Outside of NYBPM Area

# 118. PMODE1       Primary Mode
# 1. School Bus
# 2. Taxi or Group Ride
# 3. Commuter Rail
# 4. Express Bus
# 5. Ferry
# 6. Light Rail/LRT
# 7. Subway
# 8. Other Bus
# 9. Local Bus
# 10. Auto Passenger
# 11. Auto Driver
# 12. Bike
# 13. Walk
# 97. Other

# 125. ODTPURP      Origin to Destination Primary Trip Purpose (Aggregate)
# 1. Home to Work
# 2. Home to School
# 3. Home to Other
# 4. Work to Home
# 5. School to Home
# 6. Other to Home
# 7. Work to Other
# 8. School to Other 
# 9. All other trips

# ---. ODTPURP_RE1    Origin to Destination Primary Trip Purpose (Aggregate) - Relabel 1
# 0. Non-work Trips
# 1. Work Trips

# 130. TOD_R        Time of Departure Range
# 1. AM PEAK  (6am - 10am)
# 2. MIDDAY   (1-am - 4pm)
# 3. PM PEAK  (4pm - 8pm)
# 4. EVENING  (8pm - 12am)
# 5. OWL      (12am - 6am)

# 138. INCOM_R      Household Income Range
# 1. Below $30K
# 2. $30K - $74.9K
# 3. $75K - $99.9K
# 4. $ 100K +
# 9. Did not provide

# ---. PMODE1_RE1   Primary Mode - Relabel 1
# 1. Auto
# 2. Bus
# 3. Rail
# 4. Commuter Rail
# 5. Ferry
# 6. Bike
# 7. Walk
# Other

# ---. PMODE1_RE2   Primary Mode - Relabel 2
# 1. Auto
# 2. Transit
# 3. Other







#################################################################################


# Explore Public_LInked Data more

# 17. HTRACT     Home Census Tract (2010)
# 18. OTAZ       Origin TAZ (NYMTC; WGS84)
# 19. DTAZ       Destination TAZ (NYMTC; WGS84)
# 79. TRPDIST_HN    Hwy Network Trip Distance (Miles)
# 80. TRPDUR_HN     Trip Duration (Minutes) - Highway Network 
# 81. OTRACT        Origin Tract 2010 (WGS84)
# 82. DTRACT        Destination Tract 2010 (WGS84)
# 123. WORK_PURP    Tour Purpose; Work vs Non-work. Based on TOUR_PURP
# 129. WORKTRIP     Trip Purpose; Dummy for work trip; Re-categorized varialbe based on ODTPURP2_R


#################################################################################


# All Data Description
# 1.  PLSAM      Place Unique Identifier  11 digits wide = SAMPN + PERNO + PLANO
# 2.  SAMPN      Household Identifier     7 digits wide
# 3.  PERNO      Person Number            2 digits wide
# 4.  PLANO      Place Number             2 digits wide
# 5.  PERTYPE    Person Type Code         e.g. Full Time, Part Time, University Student...
# 6.  ORIG_HOME  If Home is Origin        0 - No / 1 - Yes
# 7.  GTYPE      GPS Record Type          1 - GPS / 2 - Non-GPS
# 8.  HHSIZE     Household Size
# 9.  DEST_HOME  If Home is Destination   0- No / 1 - Yes
# 10. LTRIPNO    Linked Trip Number
# 11. TOUR_ID    Tour ID  
# 12. SUBT_ID    Subtour ID               
# 13. DTYPE      Destination Type Code    e.g. Primary destination, Outbound stop, Inbound Stop or Home...
# 14. TOUR_PURP  Tour Purpose             e.g. Home, Work, University, School...
# 15. DOW        Day of Week of Travel    e.g. Monday, Tuesday, Wednesday...
# 16. HTAZ       Home TAZ (NYMTC)
                # 17. HTRACT     Home Census Tract (2010)
                # 18. OTAZ       Origin TAZ (NYMTC; WGS84)
                # 19. DTAZ       Destination TAZ (NYMTC; WGS84)
# 20. NO_TAZ     No TAZ flag for origin and destination       e.g. 0 - TAZ in both origin and destination / 1 - TAZ in either origin and destination / 2 - No TAZ in both origin and destination
# 21. OTPURP     Origin Primary Trip Purpose
# 22. DTPURP     Destination Primary Trip Purpose    
# 23. OTPURP_AGG    Origin Primary Trip Purpose Aggregated
# 24. DTPURP_AGG    Destination Primary  Trip Purpose Aggregated
# 25. DTPURP2       Secondary Trip Purpose
# 26. LTMODE_AGG    Aggregate Linked Trip Mode
# 27. MODE_SAMP     Linked Trip Mode Code
# 28. TOTTR         Total Number of People in the Trip
# 29. HHMEM         Household Members
# 30. PER1          Person Number on Trip
# 31. PER2          Person Number on Trip
# 32. PER3          Person Number on Trip
# 33. PER4          Person Number on Trip
# 34. PER5          Person Number on Trip
# 35. NONHH         Non-Household Member
# 36. VEHNO         Vehicle Number
# 37. DYGOV         Exit Vehicle      
# 38. PLOC          Parking Location
# 39. PRKTY         Parking Description
# 40. PAYPK         Pay to Park
# 41. PKAMT         Pay to Park Amount
# 42. PKUNT         Pay Unit
# 43. TOLFT         Paid fee for a toll read or bridge/tunnel on your trip
# 44. TLONB         Number of Toll Facilities Used
# 45. TLFC1         Toll Road or Bridge/Tunnel
# 46. TOPN1         Name of Toll Facility
# 47. TOLE1         Interchange Used to Enter Facility
# 48. TOLX1         Interchange Used to Exit Facility
# 49. TLLC1         Amount of Toll Fee Paid
# 50. TLFR1         Toll Payment Method
# 51. TLFC2         Toll Road or Bridge Tunnel
# 52. TOPN2         Name of Toll Facility
# 53. TOLE2         Interchange Used to Enter Facility
# 54. TOLX2         Interchange Used to Exit Facility
# 55. TLLC2         Amount of Toll Fee Paid
# 56. TLFR2         Toll Payment Method
# 57. TLFC3         Toll Road or Bridge Tunnel
# 58. TOPN3         Name of Toll Facility
# 59. TOLE3         Interchange Used to Enter Facility
# 60. TOLX3         Interchange Used to Exit Facility
# 61. TLLC3         Amount of Toll Fee Paid
# 62. TLFR3         Toll Payment Method
# 63. O_TLFR3       Other, Toll Payment Method
# 64. ROUTE         Route/Line Number of Transit Trips
# 65. SERVC         Transit Service
# 66. FARE          Transit Fare Type
# 67. FAREC         Transit Fare Cash
# 68. BUSPS         Transit Pass Type
# 69. MTABP         Type of Unlimited Ride Metrocard
# 70. BPFAR         Transit Pass Cost
# 71. FRBAS         Transit Pass Unit
# 72. TRP_DEP_HR    Trip Departure Hours
# 73. TRP_DEP_MIN   Trip Departure Minute
# 74. TRP_ARR_HR    Trip Arrival Hours
# 75. TRP_ARR_MIN   Trip Arrival Minutes
                # 76. TRPDUR        Trip Duration
# 77. ACTDUR        Activity Duration
                # 78. TRIPDIST      Trip Distance in Miles: Calculated Euclidean distance (bird's flight)
                # 79. TRPDIST_HN    Hwy Network Trip Distance (Miles)
                # 80. TRPDUR_HN     Trip Duration (Minutes) - Highway Network 
                # 81. OTRACT        Origin Tract 2010 (WGS84)
                # 82. DTRACT        Destination Tract 2010 (WGS84)
# 83. OMCD          Origin MCD
# 84. DMCD          Destination MCD
# 85. OZIP          Origin Zip
# 86. DZIP          Destination Zip
# 87. UNIQUEID2     UID for LinkedTrips
# 88. UNIQUEID3     UID for Tours
# 89. UNIQUEID4     UID for Subtours
# 90. HH1           DOMAIN
# 91. HH_WHT2       Level 2 Weights
# 92. TOURFAC       Tour Correction Factors
# 93. WHT_FAC3      Level 3 Weights
# 94. WHT_FAC3_VOCC Level 3 Weights
        # 95. HCOUNTY       Home County FIPS
# 96. HCITY_MCD     Home MCD
# 97. HSTATE        Home State
# 98. HZIP          Home Zip
# 99. HNYC          Home is in NYC
# 100. HMPO         Home MPO
# 101. ONYC         Origin is in NYC
# 102. OMPO         Origin MPO
# 103. DNYC         Destination in NYC
# 104. DMPO         Destination MPO
# 105. GEO_GROUP1   Home County Group: Level 1
# 106. GEO_GROUP2   Home Regional Boundaries: Level 2
# 107. GEO_GROUP3   Home Political Boundaries: Level 2
# 108. GEO_GROUP1_O Origin County Group: Level 1
# 109. GEO_GROUP2_O Origin Regional Boundaries: Level 2
# 110. GEO_GROUP3_O Origin Political Boundaries: Level 3
# 111. GEO_GROUP1_D Destination County Group: Level 1
# 112. GEO_GROUP2_D Destination Regional Boundaries: Level 2
# 113. GEO_GROUP3_D Destination Political Boundaries: Level 3
# 114. ADJ_COUNTY   Trip Destination: county group (adjacent & non-adjacent counties)
        # 115. OCOUNTY      Origin County FIPS
        # 116. DCOUNTY      Destination County FIPS
# 117. PMODE        Primary Mode
        # 118. PMODE1       Primary Mode        e.g. School Bus, Taxi or Group Ride, Commuter Rail....
# 119. PAMODE       Primary Access Mode for Premium Transit
# 120. PMODE_R      Primary Mode Range
# 121. PMODE_R2     Primary Mode Range2
# 122. PMODE_R3     Primary Mode Range3
                # 123. WORK_PURP    Tour Purpose; Work vs Non-work. Based on TOUR_PURP
# 124. DTPURP_R     Destination Primary Trip Purpose
        # 125. ODTPURP      Origin to Destination Primary Trip Purpose (Aggregate)        e.g. Home, Work, School, Others...
# 126. ODTPURP1     Origin-Destination Primary Trip Purpose 1
# 127. ODTPURP2     Origin-Destination Primary Trip Purpose 2
# 128. ODTPURP2_R   Origin to Destination Primary Trip Purpose 2
                # 129. WORKTRIP     Trip Purpose; Dummy for work trip; Re-categorized varialbe based on ODTPURP2_R
        # 130. TOD_R        Time of Departure Range       e.g. AM PEAK, MIDDAY, PM PEAK...
# 131. TOD_R1       Time of Departure Range 1
# 132. TOD_PEAK     Time of Departure; Non-Peak & Peak
# 133. TOTTR_R      Total Number of Persons in the Trip
# 134. TRPDUR_R     Trip Duration Range (Miles)
# 135. TRIPDIST_R1  Trip Distance Range 1
# 136. TRIPDIST_R2  Trip Distance Range 2
# 137. HHSIZ_R      Household Size Range
        # 138. INCOM_R      Household Income Range        e.g. Below $30K, $30K-74.9K, $75K-99.9K, ...
# 139. HHVEH_R      Number of Household Vehicle, Range

