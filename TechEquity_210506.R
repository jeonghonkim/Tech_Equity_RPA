...
data_Tracts_ny <- acs19_5yr_ny %>%
  ...
  mutate(
    ...
    RAC_AIN_PCT = (RAC_NHS_AIN / RAC_TTL) * 100,
    RAC_ASN_PCT = (RAC_NHS_ASN / RAC_TTL) * 100,
    RAC_HSP_PCT = (RAC_YHS_TTL / RAC_TTL) * 100,
    RAC_OTH_PCT = (RAC_NHS_OTH / RAC_TTL) * 100,
  ...
  mutate_at(vars(TEC_INT_PCT, TEC_BRD_PCT, TEC_SMR_PCT, TEC_CDP_PCT, RAC_WHT_PCT, RAC_AFA_PCT, RAC_AIN_PCT, 
                 RAC_ASN_PCT, RAC_HSP_PCT, RAC_OTH_PCT, EDU_LV1_PCT, EDU_LV2_PCT, EDU_LV3_PCT, EDU_LV4_PCT,
                 LAB_YES_PCT, EMP_NOT_PCT), 
            funs(round(., 1))) %>%
  select(GEOID, TEC_INT_PCT:TEC_CDP_PCT, AGE_MED, RAC_WHT_PCT:RAC_OTH_PCT, EDU_LV1_PCT:EDU_LV4_PCT, EMP_NOT_PCT, 
         LAB_YES_PCT, INC_MED, OWC_MED, CRT_MED, GRT_MED) %>%
...
# Export to csv file
write.csv(data_Tracts_tri, file = 'G:/Shared drives/Projects/5035_Tech Equity/Data/CSV/data_Tracts_csv.csv')
data_Tracts <- read.csv(file = 'G:/Shared drives/Projects/5035_Tech Equity/Data/CSV/data_Tracts_csv.csv')
...
st_write(geoData_Tracts,
         "G:/Shared drives/Projects/5035_Tech Equity/Data/Shapefile/IPUMS_Tracts_shp.shp", 
         driver = "ESRI Shapefile")
View(geoData_Tracts)
